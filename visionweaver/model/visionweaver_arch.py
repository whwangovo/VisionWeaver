...
class VisionWeaverMetaModel:

    def __init__(self, config):
        super(VisionWeaverMetaModel, self).__init__(config)

        # will not have this args when init
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        ...
        # llm hidden_size to config.hidden_size
        ...
        if pretrain_mm_mlp_adapter is not None:
            pretrained_weights = torch.load(pretrain_mm_mlp_adapter, map_location='cpu', weights_only=True)
            def get_w(weights, keyword):
                return {k.split(keyword + '.')[1]: v for k, v in weights.items() if keyword in k}

            self.vision_tower.load_state_dict(get_w(pretrained_weights, 'vision_tower'), strict=False)
            self.mm_projector.load_state_dict(get_w(pretrained_weights, 'mm_projector'))
...
class VisionWeaverMetaForCausalLM(ABC):
    ...
    def encode_images(self, images, input_embeds=None):
        # rank0_print(f"images: {images.shape}")
        image_features = self.get_model().get_vision_tower()(images)
        # rank0_print(f"image_features: {image_features.shape}")
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        # rank0_print(f"input_ids: {input_ids}")
        ...
        if type(images) is list or images.ndim == 5:
            ...
            pure_input_ids = self.get_pure_input(input_ids)
            # rank0_print(f"input_ids: {input_ids}")
            # rank0_print(f"pure_input_ids: {pure_input_ids}")
            
            # input_embeds = self.get_model().embed_tokens(pure_input_ids).clone()
            # image_features = self.encode_images(images, input_embeds)
